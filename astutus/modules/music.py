...
class VoiceConnectionError(cmd.CommandError):
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(
        cls, ctx, search: str, *, loop, download=False, search_only=False
    ):
...
class Music(cmd.Cog):
    ...
    async def __local_check(self, ctx):
        ...
        if not ctx.guild:
            raise cmd.NoPrivateMessage
        return True

    async def __error(self, ctx, error):
        """A local error handler for all errors arising from commands in this cog."""
        if isinstance(error, cmd.NoPrivateMessage):
            try:
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if vc is not None:
            ...
            try:
                ...
                raise cmd.BadArgument("Opus is not yet loaded")
    ...
    @cmd.command(name="queue", aliases=["q", "playlist"])
